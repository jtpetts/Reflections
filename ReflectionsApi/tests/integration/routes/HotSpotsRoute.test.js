...
describe('/api/Maps', () => {
    ...
    async function insertMaps() {
        ...
        let imap1 = new MapModel({
            ...
            hotSpots: [{ "x": 371, "y": 173, "name": "Riverside", "description": "A mountain town.", "zoomName": "Riverside" }, { "x": 74, "y": 210, "name": "Woodvine", "description": "Where Sophie lives.", "zoomName": "Woodvine" }, { "name": "Kernville", "description": "Where they were attempting to connect the train to.", "zoomName": "", "x": 335, "y": 33 }]
        });
        await imap1.save();
        ...
        imap2 = new MapModel({
            ...
            hotSpots: [{ "x": 78, "y": 221, "name": "Gas Station", "description": "They provisioned themselves for a potentially long ordeal.", "zoomName": "" }, { "x": 251, "y": 208, "name": "Visitor's Center", "description": "They met a park ranger and Joe Condor here.", "zoomName": "" }, { "name": "Antique Shop", "description": "Ambrose, the proprietor, attempted to sell them rusty nails here.", "zoomName": "", "x": 375, "y": 243 }, { "name": "The bridge", "description": "They crossed the bridge into spriggan territory. The seemingly ordinary bridge attempted to drop them into the gorge.", "zoomName": "", "x": 249, "y": 108 }]
        });
        await imap2.save();
        ...
        imap3 = new MapModel({
            ...
            hotSpots: [{ "x": 243, "y": 208, "name": "Marais", "description": "Where the spriggans live.", "zoomName": "" }, { "x": 201, "y": 386, "name": "Woodvine", "description": "Where Sophie lives.", "zoomName": "" }, { "name": "Downtown", "description": "Haverston took Sophie to a Mayoral dinner at the Grand Hotel.", "zoomName": "", "x": 0, "y": 0 }]
        });
        await imap3.save();
    ...
    describe('GET /:mapId/hotspots/:hotSpotId', () => {
        ...
        beforeEach(async () => {
            ...
            AntiqueShopHotSpot = _.find(AntiqueShopMap.hotSpots, { name: 'Antique Shop' });
        ...
        async function execute() {
            return response = await request(server)
                .get(`/api/Maps/${targetMapId}/hotspots/${targetHotSpotId}`);
        }
    ...
    describe('POST /:mapId/hotspots', () => {
        ...
        beforeEach(async () => {
            token = new UserModel({ name: 'siby', email: 'siby@gmail.com' }).generateAuthToken();
            initialMaps = await insertMaps();
        ...
        it('should create a hotspot and initialize the zoomId if everything is correct', async () => {
            ...
            expect(response.body.name).toBe(trainXingName);
            expect(response.body._id.str).toBe(insertedTrainXingHotSpot._id.str);
            expect(response.body.zoomId.str).toBe(insertedTrainXingHotSpot.zoomId.str);
        });
    ...
    describe('DELETE /:mapId/hotspots/:hotSpotId', () => {
        ...
        beforeEach(async () => {
            ...
            // set up the GET on the antique shop
            riversideMap = _.find(initialMaps, { name: 'Riverside' });
            antiqueShopHotSpot = _.find(riversideMap.hotSpots, { name: 'Antique Shop' });
            targetMapId = riversideMap._id;
            targetHotSpotId = antiqueShopHotSpot._id;
        });
