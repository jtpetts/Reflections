...
const { validateUser, generateAuthToken, UserModel } = require('../models/UserModel');
...
router.get('/', asyncMiddleware(async (request, response) => {
    ...
    if (users == null)
        return response.status(404).send('There are no users!');
...
router.get('/:id', asyncMiddleware( async (request, response) => {
    ...
    if (user == null)
        return response.status(404).send('The user with the given ID was not found');

    response.send(user);
}));

//__________________________________________________________________________________________________________
router.get('/email/:email', asyncMiddleware(async (request, response) => {

    const user = await UserModel.findOne({ email: request.params.email });
    if (user == null)
        return response.status(404).send('The user with the given email was not found');
...
router.post('/', asyncMiddleware(async (request, response) => {
    ...
    user = new UserModel(_.pick(request.body, ['name', 'email', 'password']));
...
//__________________________________________________________________________________________________________
router.delete('/:id', asyncMiddleware(async (request, response) => {
